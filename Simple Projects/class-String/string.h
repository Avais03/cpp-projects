...
public:
    ...
    String (const char* string) : capacity(strlen(string) + 1),
                                  sz(strlen(string)),
                                  str(new char[capacity]) { //Constructor by C-style string (const char*).
    ...
    [[nodiscard]][[maybe_unused]] char front () const { // get front character of the const string
        return *str;
    }
    ...
    [[nodiscard]][[maybe_unused]] char back () const { // get back character of the const string
        return *(str + sz - 1);
    }
    ...
    /*String& operator = (const String& other) { // Copy-and-swap idiom++
        if (this != &other) {
            String copy = other;
            swap(copy);
        }
